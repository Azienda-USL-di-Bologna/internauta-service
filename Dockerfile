...
RUN cd next-spring-data-rest-framework && mvn -q clean install -DskipTests
...
RUN cd jenesis-projections-generator && mvn -q clean install
...
RUN cd internauta-utils && mvn -q --projects jpa-tools --also-make clean install
...
RUN cd internauta-model && mvn -q clean install
...
RUN cd blackbox-permessi && mvn -q clean install
