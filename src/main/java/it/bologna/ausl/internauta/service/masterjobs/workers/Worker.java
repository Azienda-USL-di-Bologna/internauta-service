 ...
 *  WorkerData: sono i dati che il worker potrà usare per svolgere il job
 *  WorkerDeferredData: nel caso di worker deferred, sono dei dati a partire dai quali, 
 *  nel momento della sua esecuzione il worker creerà i WorkerData
 * 
...
public abstract class Worker {
    ...
    protected WorkerDeferredData workerDeferredData;
     ...
     */
    public void build(WorkerData workerData) {
     ...
     */
    public void buildDeferred(WorkerDeferredData workerDeferredData) {
     ...
     * @return 
     */
    public WorkerDataInterface getData() {
        if (deferred)
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor = Throwable.class)
    protected abstract WorkerResult doRealWork() throws MasterjobsWorkerException;
    
    /**
     * torna il nome del job, con cui sarà scritto in tabella e sarà usato per individuarlo dalla tabella nel momento in cui deve
     * essere eseguito
