...
import it.bologna.ausl.internauta.service.repositories.scripta.SequenzaRegistroRepository;
...
//    private Integer anno = 2024;
    ...
    @Autowired
    private SequenzaRegistroRepository sequenzaRegistroRepository;
    ...
    public JobWorkerResult doRealWork() throws MasterjobsWorkerException {
        ...
        try {
            ...
            //Creo la sequenza per il registo fascicolo dell'azienda
            creaSequenzaDelFascicoloPerAzienda(idAzienda);
     ...
     *
     * @param numeroArchivio Letteralmente il numero con cui l'archivio verrà
     * numerato
     * @param nomeFascicoloSpeciale L'oggetto dell'archivio
     * @param fascicoloPadre Il padre dell'archivio che sto per creare, null se
     * sono un livello 1
...
//            entityManager.refresh(newArchivio); 
    ...
    /**
     * Imposto gli attori (creatore, responsabile e vicario) di default per gli
     * archivi speciali
     *
    ...
    /**
     * Controllo se esiste già almeno un archivio di tipo attività nell'anno
     * corrente. (Do per scontato che sono la prima cosa a partire ad inizio
     * anno quindi se ne trovo almeno uno c'è un problema.)
     *
    ...
    /**
     * Controllo se esiste già almeno un archivio nell'anno corrente. (Do per
     * scontato che sono la prima cosa a partire ad inizio anno quindi se ne
     * trovo almeno uno c'è un problema.)
     *
     * @return true se ne trovo false altrimenti
     */
    ...
    /**
     * Pesco e ciclo gli archivi da duplicare e per ognuno di essi chiamo la
     * funzione apposta.
     *
     * @throws Exception
     */
...
//            }
    ...
    /**
     * Duplico realmente l'archivio passato come figlio di uno specifico
     * archivio (archivioDestinazione) e relativi attori.
     *
     * @param archivioDaCopiare L'archivio passato da copiare a cui vanno
     * apportate alcune modifiche.
     * @param archivioDestinazione L'archivio destinazione in cui il nuovo
     * archivio verrà messo come figlio.
    ...
    private void creaSequenzaDelFascicoloPerAzienda(Integer idAzienda) throws MasterjobsWorkerException {
        ...
        QSequenzaRegistro qSequenzaRegistro = QSequenzaRegistro.sequenzaRegistro;
        BooleanExpression filtroSequenza = qSequenzaRegistro.idRegistro.id.eq(registro.getId()).and(qSequenzaRegistro.anno.eq(anno));
        Optional<SequenzaRegistro> sequenzaRegistroOpt = sequenzaRegistroRepository.findOne(filtroSequenza);
        if (!sequenzaRegistroOpt.isPresent()) {
            SequenzaRegistro sequenzeRegistriNuovoAnno = new SequenzaRegistro();
            sequenzeRegistriNuovoAnno.setAnno(anno);
            sequenzeRegistriNuovoAnno.setIdRegistro(registro);
            sequenzeRegistriNuovoAnno.setUltimoNumeroStaccato(1);
            entityManager.persist(sequenzeRegistriNuovoAnno);
            entityManager.flush();
            entityManager.refresh(sequenzeRegistriNuovoAnno);
        } else {
            SequenzaRegistro sequenzaRegistroFascicolo = sequenzaRegistroOpt.get();
            if (!sequenzaRegistroFascicolo.getUltimoNumeroStaccato().equals(1)){
                sequenzaRegistroFascicolo.setUltimoNumeroStaccato(1);
                entityManager.persist(sequenzaRegistroFascicolo);
                entityManager.flush();
                entityManager.refresh(sequenzaRegistroFascicolo);
            }
        }
    }
}
