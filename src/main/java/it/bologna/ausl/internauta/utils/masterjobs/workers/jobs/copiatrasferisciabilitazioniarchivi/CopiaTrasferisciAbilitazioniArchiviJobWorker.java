...
/**
 * C'è un bug che ho deciso di lasciare. Se l'utente sorgente ha un un MODIFICA propagato, e la destinazione ha un ELIMINA,
 * Allora dovrei lasciare l'elimina e dare il modifica in tutti i sottofascicoli, purché non abbiano anche li un permesso più alto.
 * @author gusgus
 */
@MasterjobsWorker
...
//        Struttura strutturaVeicolante = strutturaRepository.getById(strutturaVeicolanteWithPlainFields.getId());
        ...
        List<PermessoEntitaStoredProcedure> permessiBlackBoxPersonaSorgente = getPermessiBlackBox(personaSorgente, false, null);
...
//            }
            ...
            if (predicatoPiuAltoDestinazione != null) {
                if (predicatoPiuAltoSorgente.equals(predicatoPiuAltoDestinazione)) {
                    ...
                    if (!(predicatoSorgentePropagato && !predicatoDestinazionePropagato)) {
                        ...
                        predicatoPiuAltoDestinazione.equals(PermessoArchivio.Predicato.ELIMINA.toString()) ||
            ...
            if (devoInserireIlNuovoPermesso) {
                ...
                if (oggettoneDestinazione != null) {
                     ...
                     * allora lo prendo, tolgo gli eventuali permessi di elimina, modifica, visualizza,
                     * poi inserisco il nuovo permesso, capendo se propagato o meno vedendo la sorgente.
                    */
...
//        return !(oraCorrente.isAfter(inizioOrario) && oraCorrente.isBefore(fineOrario));
    ...
    /**
     * Per la persona passata vengono recuperati tutti i permessi su archivi non virtuali
     * in cui la persona ha permesso con predicato tra ELIMINA, MODIFICA, VISUALIZZA.
     * Se viene passato a true tuttiPredicati, allora vengono aggiunti ulteiori predicati di filtro 
     * e se viene passato un elenco archivi allora questi vengono aggiunti come filtro sugli oggetti
     * @param persona
     * @return
     * @throws BlackBoxPermissionException 
     */
    private List<PermessoEntitaStoredProcedure> getPermessiBlackBox(Persona persona, Boolean tuttiPredicati, List<Archivio> archivi) throws MasterjobsWorkerException {
     ...
     * più alto per quell'archivio tra ELIMINA, MODIFICA, VISUALIZZA.
     * Si presume quindi che il soggetto non sia importante (sia sempre lo stesso)
     * @param oggettoneList
     * @return 
     */
    ...
    /**
     * Torna una mappa in cui la chiave è l'id provenienza dell'oggetto
     * e il valore è il PermessoEntitaStoredProcedure corrispondente 
     * @param oggettoneList
     * @return 
     */
